...
- News, quotes, fundamentals, and each stock's last-reported-earnings actuals come from Finnhub's free tier (60 calls/min, no published daily cap) — 201 requests/run (50 quotes + 50 metrics + 50 earnings actuals + 50 per-ticker company-news + 1 general-news), paced ~1.1s apart to stay under the per-minute limit, refreshed every hour. Per-ticker company-news exists alongside the general feed because the general feed rarely mentions small/mid-cap tickers by name — without it, most of the 50 tickers would show zero news even when the pipeline is working correctly (confirmed in production: only 4-6 of 50 tickers ever got covered before this was added).
...
- Each stock is tagged with an editorial sub-sector (Semis / Software y nube / Mega-cap — `SUBSECTOR` in `pipeline/fetch_and_curate.py`), shown next to the company name in the ledger and filterable via a second row of chips that combines with the existing sentiment/favorites filter (AND, not OR). Not a rigorous GICS classification — a judgment call to make the 50-ticker list scannable, not a research tool.
- Favorites get a client-side price-move alert: once per page load, each favorited ticker's current price is compared against a `localStorage` snapshot from the visitor's last visit (not the daily change), and a toast summarizes any that moved ≥5% since then. No backend, no account — the threshold is fixed and the snapshot is overwritten after every check so the next visit compares against this one.
- A "Tu selección" section surfaces favorited tickers right after the hero, separate from the "Favoritas" filter chip inside the ledger further down. Hidden until at least one ticker is favorited; the sector-wide hero stats above it always reflect the full 50-ticker universe, never a personal subset.
...
- Not yet done: `Strict-Transport-Security` and `X-Frame-Options`/`frame-ancestors` need a real HTTP header (a `<meta>` CSP can't carry `frame-ancestors`) — Vercel supports this via `vercel.json` `headers`, so this is now straightforward to add, just not yet done.
