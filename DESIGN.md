...
### Stock Modal Banner + Avatar
- **Purpose:** the modal opened straight into a wall of text (ticker, name, price, tag) with no visual anchor at the top. Reference: pelositracker.app's profile-style header — a gradient banner strip with a circular avatar overlapping its bottom edge — adapted here to give the modal a identity moment before the numbers start, without pretending to show a real company photo.
- **Structure:** an 88px-tall banner (`linear-gradient(135deg, var(--accent), var(--accent-dim))`) spans the modal's full bleed width via the same negative-margin technique the modal's hero image already uses elsewhere (`margin: -32px -28px 28px`, matching `.modal-hero-image`) — reused verbatim rather than a second full-bleed rule invented for this one case. A 56px circular avatar sits absolutely positioned, overlapping the banner's bottom edge by half its height, bordered in `--panel` so it reads as sitting on top of the banner rather than merging into it.
- **Avatar content is honest, not decorative:** it shows the ticker's first letter (`stock.ticker.charAt(0)`), styled in the label voice (`--font-data`, bold) — never a fabricated logo or stock photo, consistent with the project's rule against inventing data that isn't real.
- **Palette discipline:** the gradient uses only `var(--accent)`/`var(--accent-dim)`, the same two tokens every other accent surface on the page draws from — no new hue introduced for this component, keeping the One-Accent Rule intact even though the banner is the single largest block of accent color on the page.
...
- **Restraint:** one instance, one location, low opacity. Kept deliberately singular so it reads as a brand touch, not a pattern to repeat — see the Hero Radial Glow below for the one other decorative graphic on the page, and why the two don't compete.

### Hero Radial Glow (decorative)
- **Purpose:** the second decorative (non-data-bearing) graphic on the page, added after studying pelositracker.app's dark hero — a soft radial glow behind the hero's top-left content in dark mode only. Where the Pulse Rings are a small, literal brand mark tucked in one card's corner, this is an ambient light source behind the whole hero — the two read as different scales of the same idea (a glow and a ripple) rather than duplicates.
- **Implementation:** `.hero::before`, a single `radial-gradient(50% 60% at 28% 30%, var(--accent-dim), transparent 70%)` pseudo-element, `position: absolute; z-index: -1` inside the hero's own stacking context (`.hero` gains `position: relative; z-index: 0` to contain it, the same containment pattern the Pulse Rings use on `.hero-card`). `pointer-events: none`, no `aria-hidden` needed since a `::before` pseudo-element is never exposed to the accessibility tree at all.
- **Dark-mode only:** `opacity: 0` on the base rule, flipped to `1` only inside `:root[data-theme="dark"]` and the `prefers-color-scheme: dark` media-query fallback — the exact two-rule pattern already established for `.theme-toggle`'s sun/moon icon swap, reused rather than inventing a third way to gate dark-only styling. In light mode the hero stays flat, matching the page's existing restraint there; the glow is a dark-mode-specific mood, not a permanent fixture.
- **Palette discipline:** `var(--accent-dim)` only — the same token the Pulse Rings, the Stock Modal Banner, and every other accent surface already draw from. No new hue for the glow.

### Floating Scroll-Top Button
- **Purpose:** the page grew tall enough (six sections, a ledger, a calendar, a history archive) that returning to the hero meant a long manual scroll. Reference: pelositracker.app's floating circular action button in the bottom corner — adapted here for scroll-to-top since the site has no cart/chat/share action that would want that slot instead.
- **Structure:** a 42px circular button, `position: fixed; right: 20px; bottom: 20px`, an up-chevron icon (new hand-rolled SVG, matching the existing stroke weight and rounded-cap language used throughout the page's line icons — same reasoning as the Trend Glyphs and Category Icons: no build step, no icon-library dependency). Same `--panel` background / hairline border / soft shadow recipe as other floating surfaces on the page, not a new material.
- **Visibility:** hidden (`hidden` attribute) until the visitor has scrolled past 480px, toggled by a passive `scroll` listener in `initScrollTopButton()` — it never occupies space or draws attention before there's anywhere to scroll back to.
- **Interaction:** `window.scrollTo({ top: 0, behavior: ... })`, gated through the existing `prefersReducedMotion()` helper (the same one the count-up number animation uses) rather than the CSS `scroll-behavior` mechanism the anchor-link jumps rely on — an explicit `behavior: "smooth"` passed to `scrollTo()` would otherwise override that CSS-level opt-out, so this path checks `prefersReducedMotion()` directly and falls back to `"auto"`.
...
### "Tu selección" (signature component)
- **Purpose:** a dedicated landing block for favorited tickers, distinct from the existing "Favoritas" filter chip buried inside the ledger — surfaces what the visitor actually cares about right after the hero, before they have to scroll and filter for it. Hidden entirely (`section[hidden]`) until at least one ticker is favorited; never shown empty.
- **Structure:** the exact `.hero-mover` button reused verbatim — no new card. One button per favorite, company name as the label, ticker pill + change% in the same row, wrapped in a flex-wrap row so the count is unbounded. Sits in its own section between the hero and the filters, with the border-top/margin the hero column's version needs (to separate it from the badges above) stripped, since here it already has its own heading.
- **Never touches sector-wide stats:** the hero's breadth/mover numbers above it keep reflecting the full 50-ticker universe — this section reads from the same `STOCKS`/`favorites` state everything else does, but adds a personal view, not a replacement one.
...
- **Section nav is icon-only, in a pill.** Six sections (Resumen / Empresas / Comparar / Noticias / Calendario / Historial), each a 34px circular hit target holding a 17px icon — no label text. Reference: pelositracker.app's topbar nav, which uses the same icon-pill pattern instead of a text link row (only the structural idea was taken — no copied markup or assets). Every icon is the exact SVG path already used at that section's own `<h2>` (Category Icons component), so nothing new was drawn — six existing marks, reused as nav targets instead of duplicated as new glyphs. `title`/`aria-label` on each link carry the section name for hover tooltips and screen readers, since the label is no longer in the visible text.
...
- **Mobile (`≤640px`):** rather than hide the nav entirely (the pre-existing rule, when it was six text links that didn't fit), the icon-only pill is small enough to keep — it detaches from the topbar and becomes a `position: fixed` floating bar centered at the bottom of the viewport, the same "icon dock" placement pelositracker.app uses on narrow screens. Six 34px circular targets stay comfortably thumb-reachable without crowding a phone-width topbar the way six text links did.
